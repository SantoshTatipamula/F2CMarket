          ...
          {categories.map((item) => {
            ...
            return (
              <motion.div
                variants={fadeUp}
                key={item.id}
                className="group bg-[var(--bg)] border border-[var(--border)] rounded-3xl p-6 text-center hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer"
              >
                {/* Icon Box */}
                <div
                ...
                {/* Title */}
                <h3 className="font-semibold text-[var(--text-primary)] group-hover:text-[var(--primary)] transition">
                  {item.title}
                </h3>
              </motion.div>
            );
